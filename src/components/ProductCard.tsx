  ...
  return (
    <div className="flex flex-col group cursor-pointer shrink-0 w-[75vw] sm:w-[45vw] md:w-full snap-start">
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#EAE2D6] mb-3 md:mb-6">
        ...
        <div className="hidden md:block absolute bottom-0 left-0 w-full p-4 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out z-10">
          <Link href={`/products/${node.handle}`}>
            <button 
              className="w-full py-3 bg-[#BA2461] text-[#FDFBF7] hover:bg-[#951C4D] transition-colors duration-500 font-sans tracking-widest text-xs uppercase flex justify-center items-center shadow-lg"
            >
              SELECT SIZE
            </button>
          </Link>
      ...
      <div className="flex flex-col items-center text-center w-full px-1">
        ...
        <Link href={`/products/${node.handle}`} className="w-full md:hidden">
          <button 
            className="w-full py-2 mt-2 text-[10px] sm:text-xs font-medium tracking-wide uppercase bg-[#BA2461] text-[#FDFBF7] hover:bg-[#951C4D] transition-colors"
          >
            SELECT SIZE
          </button>
        </Link>
