...
export default function PDPDesktop({ product, images, price }: { product: any, images: any[], price: any }) {
  return (
    <div className="grid grid-cols-12 gap-10 items-start">
      {/* Left: Image Gallery (Stacked Grid) */}
      <div className="col-span-7 grid grid-cols-2 gap-4">
        {images.map((edge: any, idx: number) => (
          <div key={idx} className={`relative aspect-[4/5] rounded-md overflow-hidden ${images.length % 2 !== 0 && idx === images.length - 1 ? 'col-span-2' : ''}`}>
            <Image
              src={edge.node.url}
      ...
      <div className="col-span-5 sticky top-20 flex flex-col">
        <h1 className="text-4xl lg:text-5xl font-serif text-[#27272A] mb-4 leading-tight">
          {product.title}
        </h1>
